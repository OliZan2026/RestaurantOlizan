...
(function () {
  ...
  if (tokenReset) {
    ...
    sectiuneVizitator.hidden = true; sectiuneClient.hidden = true;
  ...
  $("#btn-parola-uitata").addEventListener("click", function () {
    ...
    formParolaUitata.hidden = deschis; this.setAttribute("aria-expanded", String(!deschis));
    if (!deschis) { $("#reset-email").value = $("#login-email").value.trim(); $("#reset-email").focus(); }
  });
  formParolaUitata.addEventListener("submit", function (e) {
    e.preventDefault(); var status = $("#reset-request-status"); blocheaza(formParolaUitata, true); stare(status, "Se pregătește e-mailul…");
    cerere("/api/auth/forgot-password", { method: "POST", corp: { email: $("#reset-email").value.trim() } })
      .then(function (d) { stare(status, d.mesaj || "Verifică adresa de e-mail.", true); formParolaUitata.reset(); })
      .catch(function (err) { stare(status, err.message, false); })
      .then(function () { blocheaza(formParolaUitata, false); });
  });
  if (formParolaNoua) formParolaNoua.addEventListener("submit", function (e) {
    e.preventDefault(); var status = $("#reset-password-status"); var parola = $("#parola-noua").value;
    if (parola !== $("#parola-confirma").value) { stare(status, "Parolele introduse nu sunt identice.", false); return; }
    blocheaza(formParolaNoua, true); stare(status, "Se salvează parola…");
    cerere("/api/auth/reset-password", { method: "POST", corp: { token: tokenReset, parola: parola } })
      .then(function (d) { stare(status, d.mesaj, true); formParolaNoua.reset(); window.history.replaceState({}, "", "/cont"); setTimeout(function () { window.location.href = "/cont"; }, 1800); })
      .catch(function (err) { stare(status, err.message, false); })
  ...
  /* --- starea inițială ----------------------------------------------------- */
  if (!tokenReset) cerere("/api/auth/me")
